...
def learning_styles_report(request):
    ...
    if request.method == "POST":
        ...
        results = False
        if (re.match('\d{2}/\d{2}/\d{4}', from_date)
            and re.match('\d{2}/\d{2}/\d{4}', to_date)):
            from_date = from_date.split('/')
            from_date = datetime(int(from_date[2]),
                                 int(from_date[0]),
                                 int(from_date[1]))
            to_date = to_date.split('/')
            to_date = datetime(int(to_date[2]),
                               int(to_date[0]),
                               int(to_date[1]))
        ...
        if results:
            ...
            for result in results:
                row = (str(result.student.id),
                       get_username(result.student),
                       result.date_taken.strftime('%m/%d/%Y'),
                       result.learning_style,
                       str(result.kinesthetic_score),
                       str(result.visual_score),
                       str(result.auditory_score))
                rows.append(row)
            i = 0
            ...
            return direct_to_template(request, 'reports/report_form.html', {
                'error': 'No results found for that date range.',
                'learning': True,
                'report_type': 'Learning Styles'
            })

    return direct_to_template(request, 'reports/report_form.html', {
        'learning': True,
        'report_type': 'Learning Style'
    })
    ...
    if request.method == "POST":
        ...
        if (re.match('\d{2}/\d{2}/\d{4}', from_date)
            ...
            results = PersonalityTypeResult.objects.filter(
                date_taken__range=(datetime.combine(from_date, time.min),
                                   datetime.combine(to_date, time.max)))
        if results:
            ...
            for result in results:
                row = (str(result.student.id),
                       ...
                       str(result.first_category_score),
                       str(result.second_category_score),
                       str(result.third_category_score),
                       str(result.fourth_category_score))
                rows.append(row)
            i = 0
            for column in columns:
                ws.write(0, i, column)
